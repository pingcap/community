...
| sink: fix kafka max message size inaccurate issue            | ❌                 | ❌                                 | ❌                    | Change the default value of Kafka Sink max-message-bytes to 1 MB to prevent TiCDC from sending too large messages to Kafka clusters |
...
| Better err msg when PD endpoint missing certificate          | ✅                 | ✅                                 | ✅                    | Improve the error message returned when a PD endpoint misses the certificate |
